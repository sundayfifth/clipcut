# clipcut

เครื่องมือ local สำหรับแปลงคลิปคอนเทนต์สุขภาพภาษาไทย จาก 16:9 (YouTube) ให้เป็น 9:16 (TikTok) แบบกึ่งอัตโนมัติ

รับคลิปต้นฉบับ → วิเคราะห์ให้ว่าซีนไหน crop ได้ ซีนไหนต้องย่อ+เติมพื้นหลัง → render ไฟล์ 9:16 ที่พร้อมใช้เกือบสมบูรณ์ พร้อม checklist บอกว่ามี text/graphic ตรงไหนบ้างที่ต้องทำใหม่แล้วเติมเองใน CapCut/Hyperframe

**สิ่งที่เครื่องมือนี้ต้องทำ**
- รับ input ได้ทั้งไฟล์ในเครื่องและ URL YouTube
- แบ่ง shot แล้วตัดสินใจต่อ shot ว่า crop (ตามตัวคน) หรือ ย่อ+เติมพื้นหลัง
- ตรวจจับ text/graphic ที่ burn มากับต้นฉบับ แล้วรายงาน timecode + ตำแหน่ง + ข้อความ — **ไม่ลบและไม่ทับให้เอง**
- เลือกได้ตอนใช้ว่าจะเอาคลิปเท่าต้นฉบับ หรือตัดสั้นเฉพาะช่วงน่าสนใจ
- render ออกเป็น .mp4 9:16 + ไฟล์ checklist

## Stack

- Python 3.12 + FastAPI (backend) — `app/main.py`
- MediaPipe ObjectDetector ตรวจจับคน (**pin < 1.0** — 1.0.x crash บน macOS) โมเดลโหลดด้วย `./download-models.sh`
- Vanilla HTML/CSS/JS (frontend) — `app/static/`
- ffmpeg เป็น render engine (มีในเครื่องที่ `/opt/homebrew/bin/ffmpeg`)
- PySceneDetect — หา shot boundary
- OpenCV — อ่าน/วิเคราะห์เฟรม
- yt-dlp — โหลดคลิปจาก YouTube

**ห้ามใช้ `ultralytics` (YOLO)** — เป็น AGPL-3.0 และ Ultralytics ระบุเองว่า internal business tool แบบ clipcut ต้องซื้อ Enterprise License รวมถึงห้าม depend repo ที่ depend มันด้วย ใช้ MediaPipe (Apache 2.0) เป็นหลัก + RF-DETR รุ่น Nano/Small/Medium/Large (Apache 2.0) เมื่อต้องการ multi-person — เลี่ยงรุ่น XL/2XL ที่เป็น PML 1.0

**Output format ตัดสินแล้ว**: deliverable คือ `.mp4` 9:16 + `.srt` + checklist ไม่ใช่ project file — CapCut ไม่รองรับ import timeline จาก NLE อื่นเลย (ยืนยันจาก help center ของ CapCut เอง) แต่ import SRT ได้เป็น text clip แยกชิ้นที่แก้ได้ **ไม่ต้องทำ OTIO/EDL/FCPXML** และห้ามให้ draft-folder generation เป็นทางเดียวที่ output ออกได้ — ดู `docs/2026-08-25-research-nle-export.md`

**อ่านข้อความไทยในเฟรมด้วย Apple Vision** — `VNRecognizeTextRequest` **revision 3 + recognitionLevel `accurate` เท่านั้น**
⚠️ ค่า enum คือ `Accurate = 0`, `Fast = 1` (สลับกับที่คนเดา) · ระดับ `fast` ไม่มีภาษาไทยเลยทุก revision ถ้าเผลอใช้จะได้ผลเป็นขยะ
เรียกผ่าน `pyobjc-framework-Vision` (MIT) ไม่ต้องโหลดโมเดล ไม่มีปัญหา license · 131 ms/เฟรมบนเครื่องนี้ · ดู `docs/2026-08-27-research-thai-text-detection.md`

**ยังไม่ตัดสินใจ**: Hyperframe รองรับ import อะไร (หาข้อมูลไม่เจอเลย ต้องเปิดโปรแกรมดูเอง)

## Running it

```
source .venv/bin/activate
uvicorn app.main:app --reload
```

แล้วเปิด http://127.0.0.1:8000

test: `pytest` (ต้องมี ffmpeg — test สร้างคลิปทดสอบเอง)

**สถานะตอนนี้**: รับไฟล์/URL YouTube → แบ่งซีน (4 ระดับ) → ตรวจจับคน → ตั้งแถบตัดซับ/โลโก้ (trim หรือ blur พร้อม preview) → เลือกซีน (ค่าตั้งต้นเลือกทั้งหมด) → ตัดสิน crop/pad ต่อซีน (คนพลิกเองได้) → render mp4 9:16 + `graphics-checklist.md`
ยังไม่ทำ: ตัดสั้นเฉพาะช่วงน่าสนใจ (hook detection), ตรวจจับตำแหน่งซับอัตโนมัติ

**ซีนที่ crop แล้วข้อความหายเกิน 3 จุด จะถูกพลิกเป็น "ย่อทั้งภาพ" อัตโนมัติ** (`prefer_pad_over_losing_text()` ใน `app/plan.py`)
ย่อแล้วข้อความอยู่ครบ ไม่ต้องทำกราฟฟิกใหม่เลย แลกกับภาพเล็กลงในซีนนั้น
- วัดกับคลิปจริง: งานกราฟฟิก **42 จุด → 3 จุด** โดยพลิกแค่ 4 ซีน · อีกคลิปพลิกแค่ 1 ซีน (ไม่ over-trigger)
- **ห้ามนับโลโก้ประจำช่อง** ไม่งั้นจะพลิกทั้งคลิปเป็นย่อหมด · **ห้ามแตะซีนที่คนเลือกโหมดเอง**

**บทเรียนที่ต้องจำ**: อย่าบังคับให้ตัวคน*ทั้งตัว*อยู่ในกรอบ 9:16 — เคยเขียนแบบนั้นแล้ว 23 จาก 25 ซีนกลายเป็น pad หมด สิ่งที่ต้องอยู่ในกรอบคือ*ตำแหน่ง*ของ subject
**ข้อจำกัดที่รู้แล้ว**: สกรีนช็อตที่มีรูปคนอยู่ข้างใน detector จะเห็นเป็นคนจริงแล้ว crop จนข้อความหาย — จึงต้องมีปุ่มให้คนพลิก
**การเดาแถบซับ**: ใช้ text detection จริงแล้ว แต่ **ห้ามดูแค่ "ข้อความอยู่ล่าง"** — คลิปที่มีสกรีนช็อตเยอะจะมีข้อความทุกตำแหน่ง ทดสอบแล้วมันแนะนำให้ตัดทิ้งครึ่งจอ
สัญญาณที่ใช้ได้คือ **ตำแหน่งเดิมซ้ำๆ ข้ามหลายซีน** (`suggest_bands()` ใน `app/textdet.py`) และ **ไม่ใส่ให้อัตโนมัติ** โชว์เป็นข้อเสนอให้กดรับ

**บนเครื่องที่ไม่ใช่ Mac** — อ่านข้อความในเฟรมไม่ได้ (Apple Vision มีเฉพาะ macOS) pipeline ยังเดินครบแต่ไม่มี checklist กราฟฟิกและไม่แนะนำแถบซับ
⚠️ **ห้ามให้ checklist สรุปว่า "ไม่มีข้อความหาย" บนเครื่องแบบนี้** — มันมองไม่เห็น ไม่ใช่ไม่มี การโกหกแบบนี้แย่กว่าไม่บอกอะไรเลย (`text_detection` ใน plan + notice บนหน้าเว็บ + test ดักไว้)

## UI

ธีมเข้มเป็นหลัก (งานคือจ้อง thumbnail ทั้งวัน) accent เทียลตามโลโก้ของแบรนด์
**ใช้ token เสมอ ห้ามใส่ค่าดิบ** — `--t-*` ขนาดตัวอักษร (xs→2xl) · `--s-1..8` ระยะห่าง (ฐาน 4px) · `--scrim*` ฉากทับบนภาพ
เคยปล่อยให้สะสมจนมี 11 ขนาดตัวอักษรและ 20 ค่าระยะห่าง บางคู่ต่างกันไม่ถึง 1px — ถ้าต้องใช้ค่าใหม่ให้เพิ่มเป็น token ไม่ใช่เขียนตรงๆ
ฟอนต์ Google Sans (SIL OFL) self-host ที่ `app/static/fonts/` — **มีอักษรไทยครบ 87 ตัว** จึงใช้ family เดียวทั้ง UI
subset เหลือ 111 KB จาก 4.6 MB แต่ต้องเก็บ GPOS ไว้ ไม่งั้นวรรณยุกต์ไทยลอย (ดู `app/static/fonts/README.md`)
- การ์ดซีนสลับได้ 2 มุมมอง: **ผลลัพธ์ 9:16** (คำนวณด้วย CSS ให้ตรงกับที่ ffmpeg เรนเดอร์) กับ **ภาพต้นฉบับ** ที่วาดกรอบทับให้เห็นว่าจะเสียอะไร — อันแรกเป็นค่าตั้งต้นเพราะคนตัดต่อไม่ต้องจินตนาการเอง
- **ปรับกรอบเองได้ต่อซีน** (เฉพาะโหมดเต็มจอ): เลื่อนซ้ายขวา · ซูม · ขึ้นลง เก็บเป็น `adjust: {dx, dy, scale}` ซึ่งเป็น**ส่วนต่างจากกรอบฐาน** (`crop_base`) ไม่ใช่ทับค่าไปเลย — กรอบจึงยังขยับตามคนตาม polynomial ได้อยู่ แค่เยื้องไปทั้งเส้น
  - `derive_crop()` ใน `app/plan.py` กับ `deriveCrop()` ใน `app.js` **ต้องแก้ให้ตรงกันเสมอ** ฝั่ง JS ใช้ทำพรีวิวสดตอนลากสไลเดอร์
  - **การตัดสินใจของคนต้องรอดจากการคำนวณใหม่** — `set_bands` เก็บ `included`, `manual_mode`, `adjust` ไว้แล้วใส่กลับ ไม่งั้นขยับแถบทีเดียวงานที่ตรวจไว้หายหมด
- **ลากบนภาพเพื่อขยับกรอบ · ⌥+ล้อ เพื่อซูม** (ต้องกด Option ค้าง ไม่งั้นเลื่อนหน้าผ่านการ์ดแล้วกรอบเปลี่ยนโดยไม่ตั้งใจ) — สายตาอยู่ที่ภาพ ไม่ต้องไปมองสไลเดอร์ (เหลือสไลเดอร์แค่ซูมไว้ให้เห็นว่าปรับได้)
  - **ระหว่างลากสลับไปโชว์ภาพเต็ม + กรอบเขียว** แล้วกลับเป็น 9:16 เมื่อปล่อย — เห็นว่ากำลังดึงมาจากตรงไหน เลยไม่ต้องมีแท็บ "ภาพต้นฉบับ" แยก
  - **ทิศทางต้องเป็นแบบ "จับกรอบ"** ลากซ้ายกรอบไปซ้าย (ไม่ใช่แบบจับภาพเลื่อนแผนที่) เพราะสิ่งที่เห็นตอนลากคือกรอบบนภาพเต็ม
  - `setPointerCapture`/`releasePointerCapture` ต้องอยู่ใน try/catch — ถ้าโยน error จะบล็อกโค้ดบันทึกค่า ทำให้ค่าที่ลากไว้หายเงียบๆ
- **ภาพในการ์ดเป็น sprite หลายเฟรม** เลื่อนเมาส์ผ่านเพื่อ scrub — ตัดสินการจัดเฟรมจากเฟรมเดียวคือการเดา เพราะทั้งคนและกรอบขยับตลอดซีน
  - **ต้องใช้ `background-image` ห้ามใช้ `<img>` ที่ยืดด้วย `width:%`** — การยืด img ทำให้เบราว์เซอร์สร้าง layer เท่าขนาดที่ยืด (เคยพุ่งไป 6400px ต่อการ์ด × 25 ใบ) จนหน้าค้าง
  - `url()` ใน `style="..."` ต้องใช้ single quote ไม่งั้น attribute ขาด
- **แถบ ribbon ด้านบน** = ภาพรวมทั้งคลิปตามเวลา ความกว้างแปรตามความยาวซีน คลิกแล้วกระโดดไปการ์ดนั้น (กริดการ์ดทำให้ไม่เห็นจังหวะของคลิป)
- **undo** เก็บ snapshot ของ plan ก่อนแก้ทุกครั้ง (`Cmd/Ctrl+Z`)
- **งานเก็บลงดิสก์ทุกครั้งที่แก้** (`media/work/<id>/job.json`) ปิด server แล้วเปิดใหม่ทำต่อได้
  - **ทุก mutator ต้องเรียก `self.save(job)`** เคยลืม 2 ตัวแล้วงานที่ตรวจไว้หายตอนรีสตาร์ท — มี test `test_every_edit_survives_a_restart` ดักไว้แล้ว
- คีย์ลัดบนการ์ด: `Space` เลือก/ไม่เลือก · `C`/`P` สลับโหมด · ลูกศรซ้ายขวาย้ายการ์ด (ต้องไล่ 40+ ซีนต่อคลิป เมาส์อย่างเดียวช้าเกิน)
- **เฟรมตั้งต้นของ preview แถบซับ** เลือกจากซีนที่คนพูดนานสุด (ซับ burn อยู่ตรงนั้น) ไม่ใช่จากความหนาแน่นของขอบ — ลองแล้วมันไปเลือกสกรีนช็อต Google กับ b-roll ที่ไม่มีซับ · มีสไลเดอร์ให้เลื่อนหาเฟรมเองด้วย
- **ฟังเสียงก่อนตัดสินใจ** (`app/preview_audio.py`) — 2 แบบ: `shot` เสียงซีนนั้น · `join` เสียงรอยต่อถ้าตัดซีนนี้ออก (ท้ายซีนก่อนหน้า + หัวซีนถัดไป เฉพาะซีนที่ *ถูกเลือกไว้*)
  - **สร้างรอยต่อด้วย ffmpeg รอบเดียวแบบ encode ใหม่ ห้ามใช้ `concat -c copy`** — ไฟล์ที่ได้จาก copy เบราว์เซอร์เล่นไม่ได้ (ffprobe อ่านได้ก็จริง)
  - ใส่เฟดหัวท้ายเหมือนตอน render จริง เสียงที่ได้ยินจะได้ตรงกับไฟล์จริง
  - มี**แถบบอกตำแหน่งรอยตัด**ตอนเล่น (เส้นสีเหลือง = จุดตัด · ก่อนเส้นคือท้ายซีนเดิม หลังเส้นคือหัวซีนถัดไป) พร้อมป้าย "ท้ายซีน A ⇢ หัวซีน B" — ฟังอย่างเดียวแยกไม่ออกว่าหัวหรือท้าย
  - `JOIN_CONTEXT` มีทั้งใน `app/preview_audio.py` และ `app.js` **ต้องตรงกัน** ไม่งั้นเส้นรอยตัดชี้ผิดที่ (มี test `test_join_shape_matches_between_client_and_server` ดักไว้)
  - **`stopAudio()` ต้องถอด handler ก่อนล้าง `src`** — การเซ็ต `src=""` ทำให้ `onerror` ทำงาน แล้วจะขึ้น error ทุกครั้งที่เสียงเล่นจบปกติ
- **โลโก้ประจำช่องต้องไม่ถูกนับเป็นงานรายซีน** (`mark_persistent()` ใน `app/textdet.py`) — ข้อความที่โผล่ตำแหน่งเดิม ≥25% ของซีนถือเป็นของประจำ รายงานรวมทีเดียว เจอจริง: "โลโก้ช่อง" ขึ้น 9 ครั้งในคลิปเดียว
- **checklist จัดกลุ่มตามข้อความ** ไม่ใช่ตามซีน — กราฟฟิกอันเดียวมักกินหลายซีน · `_merge()` ตัดซ้ำทั้งแบบทับตำแหน่งและแบบข้อความเดียวกันคนละตำแหน่ง (กราฟฟิกเคลื่อนไหว)
- **เตือนตอนตัดซีนที่มีคนพูดออก** — วัด RMS ต่อซีน (`app/audio.py`) decode เสียงทั้งคลิปรอบเดียวเป็น PCM 8 kHz แล้วคำนวณในหน่วยความจำ (25 ซีนใช้ 0.09 วิ)
  - ทดสอบกับคลิปจริงแล้วแยกได้ชัด: ซีนที่มีเสียงพูด -13 ถึง -17 dB · ซีนเงียบ -37 กับ -48 dB จึงไม่ต้องลาก VAD เข้ามา
  - **เสียงบรรยายพูดคลุมทับ b-roll และสกรีนช็อตด้วย** 23 จาก 25 ซีนมีเสียงพูด — เลยเตือนเฉพาะตอนที่คน*ตัดซีนออกจริง* ไม่โชว์ค้างทุกใบ ไม่งั้นกลายเป็น noise
  - **ค่าที่คืนต้องเป็น type ของ Python ไม่ใช่ numpy** ไม่งั้น `json.dumps` เขียนไม่ได้แล้วเซฟงานไม่ได้ (พลาดมาแล้ว มี test ดักไว้)
- **เสียงตรงรอยต่อ**: เฟดเข้า-ออก 12 ms ที่หัวท้ายทุก segment กันเสียง "แป๊ก" ตอนตัดกลางคำแล้วต่อกัน (ยืนยันจาก waveform แล้ว)
- **ข้อความอธิบายต้องเป็นภาษาคน ห้ามมีตัวเลข px** — คนใช้คือคนตัดต่อ ไม่ใช่ dev
- **แถบสร้างไฟล์ด้านล่างต้องเป็น `position: sticky` ห้ามใช้ `fixed`** — sticky กินที่ใน layout เองอยู่แล้ว ไม่ต้องวัดความสูงมาเผื่อที่ เคยใช้ `fixed` + ResizeObserver วัดความสูงไปตั้ง `--bar-h` แล้วเกิด feedback loop (padding เปลี่ยน → scrollbar โผล่ → ข้อความในแถบขึ้นบรรทัดใหม่ → แถบสูงขึ้น → วัดใหม่) จนแถบโตไม่หยุดบังทั้งหน้า
- `[hidden] { display: none !important }` จำเป็น เพราะ `display: grid/flex` ทับ `[hidden]`
- contrast ผ่าน WCAG AA ทั้งสองธีม **ตรวจด้วยการคำนวณจากค่า OKLCH ตรงๆ ไม่ใช่วัดในเบราว์เซอร์** — พื้นหลังที่มี alpha ทำให้วัดในเบราว์เซอร์เพี้ยน ต่ำสุดที่มีคือ 5.08 (dark) / 4.71 (light)

## Docs

Project docs — specs, PRDs, research notes, ADRs — live under `docs/`.

อ่านก่อนเริ่มเขียน pipeline: [`docs/2026-08-25-research-auto-reframe.md`](docs/2026-08-25-research-auto-reframe.md) — กติกา crop-vs-pad, camera path smoothing, และข้อจำกัด license ที่ verify มาแล้ว

## Development rules

- แยก module ให้ขอบเขตชัด: `ingest` (รับไฟล์/URL) → `analyze` (shot, ตัวคน, text) → `plan` (สร้าง edit plan) → `render` (ffmpeg) → `report` (checklist กราฟฟิก)
- **edit plan JSON เป็น contract กลางของโปรเจกต์** ทุก module คุยกันผ่านมัน แก้ทีละ module ได้โดยไม่พังตัวอื่น เปลี่ยน schema เมื่อไหร่ให้จดเป็น ADR ใน `docs/`
- เขียน test สำหรับส่วนที่คนอื่นพึ่งพา — test behaviour ไม่ใช่ implementation
- handle error ที่เจอบ่อยให้ครบ: ไฟล์เสีย, codec ไม่รองรับ, YouTube โหลดไม่ได้, คลิปยาวจนแรมไม่พอ
- **ห้ามเขียนทับหรือลบไฟล์ใน `media/input/`** เด็ดขาด — ไฟล์ระหว่างทางไป `media/work/` ผลลัพธ์ไป `media/output/`
- งานที่กินเวลานาน (โหลด, วิเคราะห์, render) ต้องรายงาน progress กลับไปที่ UI ไม่ปล่อยให้หน้าจอค้างเงียบ
- commit ทีละก้อนเล็กที่รันได้ message เป็นภาษาอังกฤษสั้นๆ

## Notes

Real tool — others depend on this. Standards apply.

- ผู้ใช้จริงคือน้องตัดต่อ ไม่ใช่ developer — error message ต้องอ่านรู้เรื่องเป็นภาษาคน บอกว่าต้องทำอะไรต่อ
- **ซับไตเติลไม่ต้องทำในโปรเจกต์นี้** — ใช้ skill `subtitle-align` ที่มีอยู่แล้ว ต่อท้ายหลังได้ไฟล์ 9:16
- ปลายทางของ output คือ CapCut / Hyperframe เสมอ — คนยังต้องเก็บงานต่อ ไม่ต้องพยายามทำให้จบในตัว
